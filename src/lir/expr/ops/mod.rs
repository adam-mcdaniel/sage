...
pub trait AssignOp: std::fmt::Debug + std::fmt::Display {
    ...
    fn compile(&self, dst: &Expr, src: &Expr, env: &mut Env, output: &mut dyn AssemblyProgram) -> Result<(), Error> {
    ...
    fn compile_types(&self, dst: &Type, src: &Type, env: &mut Env, output: &mut dyn AssemblyProgram) -> Result<(), Error>;
}
...
pub trait UnaryOp: std::fmt::Debug + std::fmt::Display {
    ...
    fn compile_types(&self, expr: &Type, env: &mut Env, output: &mut dyn AssemblyProgram) -> Result<(), Error>;
}
...
pub trait TernaryOp: std::fmt::Debug + std::fmt::Display {
    ...
    /// Compiles the operation on the given expressions.
    fn compile(&self, a: &Expr, b: &Expr, c: &Expr, env: &mut Env, output: &mut dyn AssemblyProgram) -> Result<(), Error> {
        ...
        self.compile_types(&a.get_type(env)?, &b.get_type(env)?, &c.get_type(env)?, env, output)
    }
    /// Compiles the operation on the given types. (Generates the code for the operation.)
    fn compile_types(&self, a: &Type, b: &Type, c: &Type, env: &mut Env, output: &mut dyn AssemblyProgram) -> Result<(), Error>;
}

impl PartialEq for dyn AssignOp {
    fn eq(&self, other: &Self) -> bool {
        self.display(&Expr::ConstExpr(ConstExpr::Int(0)), &Expr::ConstExpr(ConstExpr::Int(0))).eq(&other.display(&Expr::ConstExpr(ConstExpr::Int(0)), &Expr::ConstExpr(ConstExpr::Int(0))))
    }
}

impl PartialEq for dyn UnaryOp {
    fn eq(&self, other: &Self) -> bool {
        self.display(&Expr::ConstExpr(ConstExpr::Int(0))).eq(&other.display(&Expr::ConstExpr(ConstExpr::Int(0))))
    }
}
...
impl PartialEq for dyn TernaryOp {
    fn eq(&self, other: &Self) -> bool {
        self.display(&Expr::ConstExpr(ConstExpr::Int(0)), &Expr::ConstExpr(ConstExpr::Int(0)), &Expr::ConstExpr(ConstExpr::Int(0))).eq(&other.display(&Expr::ConstExpr(ConstExpr::Int(0)), &Expr::ConstExpr(ConstExpr::Int(0)), &Expr::ConstExpr(ConstExpr::Int(0))))
    }
}
